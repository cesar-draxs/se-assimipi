    ...
    with mp_hands.Hands(
        ...
        while run_cam:
            ...
            if results.multi_hand_landmarks:
                ...
                if num_manos == 2:
                    # Si usa las dos manos, simula palabras de interacción como 'Gracias', 'Ayuda', 'Nosotros'
                    l_hand = results.multi_hand_landmarks[0].landmark[9]
                    r_hand = results.multi_hand_landmarks[1].landmark[9]
                    distancia = np.sqrt((l_hand.x - r_hand.x)**2 + (l_hand.y - r_hand.y)**2)
                    
                    if distancia < 0.2:
                        detected_word = "Gracias"
                    elif l_hand.y < 0.4 and r_hand.y < 0.4:
                        detected_word = "Ayuda"
                    else:
                        detected_word = "Hola"
                elif num_manos == 1:
                    # Si usa una mano, simula respuestas directas o preguntas
